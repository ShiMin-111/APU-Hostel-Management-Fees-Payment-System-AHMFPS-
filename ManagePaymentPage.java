...
public class ManagePaymentPage {
    ...
    JButton updateFeeButton, processPaymentButton, viewRecordButton, backButton, searchButton;
    JLabel receiptLabel;
    ...
    public ManagePaymentPage() {
        ...
        updateFeeButton.setBounds(400, 50, 120, 30);
        updateFeeButton.addActionListener(e -> saveManagementFee());
        panel.add(updateFeeButton);
        ...
        receiptScrollPane.setBounds(20, 570, 200, 70); // Set the bounds of the scroll pane
    ...
    private void saveManagementFee() {
        String fee = feeField.getText();
        try {
            double feeValue = Double.parseDouble(fee);
            if (feeValue < 0) {
                JOptionPane.showMessageDialog(frame, "Fee cannot be negative!", "Error", JOptionPane.ERROR_MESSAGE);
                return;
            }

            LocalDate currentDate = LocalDate.now();
            ManagementFee newFee = new ManagementFee(feeValue, currentDate);
            DataIO.allFee.add(newFee);
            DataIO.write();

            JOptionPane.showMessageDialog(frame, "Management fee updated successfully!");
            feeField.setText(String.format("%.2f", feeValue));
        } catch (NumberFormatException e) {
            JOptionPane.showMessageDialog(frame, "Invalid fee! Please enter a numeric value.", "Error", JOptionPane.ERROR_MESSAGE);
        }
    }
